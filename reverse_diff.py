        ...
        match expr:
            ...
            case loma_ir.ArrayAccess():
                return loma_ir.ArrayAccess(
                    var_to_differential(expr.array, var_to_dvar), expr.index, t=expr.t
                )
        ...
        match target.t:
            ...
            case loma_ir.Float():
                if overwrite:
                    ...
                    return [
                        loma_ir.CallStmt(loma_ir.Call("atomic_add", [target, deriv]))
                    ]
            case loma_ir.Struct():
                s = target.t
                stmts = []
        ...
        def mutate_assign(self, node):
            ...
            if self.has_call_expr:
                ...
                self.tmp_declare_stmts.append(
                    loma_ir.Declare(tmp_name, target.t, lineno=node.lineno)
                )
                tmp_var = loma_ir.Var(tmp_name, t=target.t)
                assign_tmp = loma_ir.Assign(tmp_var, val, lineno=node.lineno)
                assign_target = loma_ir.Assign(target, tmp_var, lineno=node.lineno)
    ...
    class ForwardPassMutator(irmutator.IRMutator):
        ...
        def mutate_declare(self, node):
            ...
            if node.t != loma_ir.Int():
                ...
                return [node, loma_ir.Declare(dvar, node.t, lineno=node.lineno)]
            else:
                return node
        ...
        def mutate_assign(self, node):
            ...
            assign_primal = loma_ir.Assign(
                node.target, self.mutate_expr(node.val), lineno=node.lineno
            )
            # backup
            ...
            cache_var_expr = loma_ir.ArrayAccess(
                loma_ir.Var(stack_name), stack_ptr_var, t=node.val.t
            )
            cache_primal = loma_ir.Assign(cache_var_expr, node.target)
            stack_advance = loma_ir.Assign(
                stack_ptr_var,
        ...
        def mutate_call_stmt(self, node):
            ...
            if call_expr.id != "atomic_add":
                ...
                args = [
                    loma_ir.Arg("target", loma_ir.Float(), loma_ir.Out()),
            ...
            for i, f_arg in enumerate(args):
                if f_arg.i == loma_ir.Out():
                    ...
                    if t_str in self.type_to_stack_and_ptr_names:
                        stack_name, stack_ptr_name = self.type_to_stack_and_ptr_names[
                            t_str
                        ]
                    else:
                        random_id = random_id_generator()
                    ...
                    stack_ptr_var = loma_ir.Var(stack_ptr_name, t=loma_ir.Int())
                    cache_var_expr = loma_ir.ArrayAccess(
                        loma_ir.Var(stack_name), stack_ptr_var, t=f_arg.t
                    )
                    ...
                    stmts.append(
                        loma_ir.Assign(
                            stack_ptr_var,
                            loma_ir.BinaryOp(
                                loma_ir.Add(), stack_ptr_var, loma_ir.ConstInt(1)
                            ),
                        )
                    )
        ...
        def mutate_while(self, node):
            ...
            stmts.append(loma_ir.Assign(loop_var, loma_ir.ConstInt(0)))
            ...
            self.type_cache_size = {}
            prev_loop_size = (
                self.parent_loop_size[-1] if len(self.parent_loop_size) > 0 else 1
            )
            ...
            if len(self.parent_loop_size) > 0:
                ...
                loop_var_stack_ptr = loma_ir.Var(
                    loop_var_stack_ptr_name, t=loma_ir.Int()
                )
                loop_var_stack = loma_ir.ArrayAccess(
                    loma_ir.Var(loop_var_stack_name), loop_var_stack_ptr
                )
                ...
                stmts.append(
                    loma_ir.Assign(
                        loop_var_stack_ptr,
                        loma_ir.BinaryOp(
                            loma_ir.Add(), loop_var_stack_ptr, loma_ir.ConstInt(1)
                        ),
                    )
                )
                ...
                self.loop_var_declare_stmts.append(
                    loma_ir.Declare(
                        loop_var_stack_name,
                        loma_ir.Array(loma_ir.Int(), self.parent_loop_size[-1]),
                    )
                )
                self.loop_var_declare_stmts.append(
                    loma_ir.Declare(loop_var_stack_ptr_name, loma_ir.Int())
                )
    ...
    class RevDiffMutator(irmutator.IRMutator):
        def mutate_function_def(self, node):
            ...
            if node.ret_type is not None:
                ...
                new_args.append(
                    loma_ir.Arg(self.return_var_id, node.ret_type, i=loma_ir.In())
                )
            ...
            for t, exprs in fm.cache_vars_list.items():
                ...
                tmp_declares.append(
                    loma_ir.Declare(
                        stack_name, loma_ir.Array(t, self.type_cache_size[t])
                    )
                )
                tmp_declares.append(
                    loma_ir.Declare(stack_ptr_name, loma_ir.Int(), loma_ir.ConstInt(0))
                )
            ...
            if node.val.t == loma_ir.Int():
                ...
                stmts.append(
                    loma_ir.Assign(
                        stack_ptr_var,
                        loma_ir.BinaryOp(
                            loma_ir.Sub(), stack_ptr_var, loma_ir.ConstInt(1)
                        ),
                    )
                )
        ...
        def mutate_ifelse(self, node):
            ...
            return loma_ir.IfElse(
                node.cond, reversed_then, reversed_else, lineno=node.lineno
            )
            ...
            if needs_restore:
                ...
                for f_arg in reversed(args):
                    if f_arg.i == loma_ir.Out():
                        ...
                        cache_var_expr, cache_target = self.cache_vars_list[
                            f_arg.t
                        ].pop()
                        stmts.append(loma_ir.Assign(cache_target, cache_var_expr))
        ...
        def mutate_while(self, node):
            ...
            if needs_pop_from_stack:
                ...
                stmts.append(
                    loma_ir.Assign(
                        loop_var_stack_ptr,
                        loma_ir.BinaryOp(
                            loma_ir.Sub(), loop_var_stack_ptr, loma_ir.ConstInt(1)
                        ),
                    )
                )
                stmts.append(
            ...
            cond = loma_ir.BinaryOp(loma_ir.Greater(), loop_var, loma_ir.ConstInt(0))
            # run the body backwards
            reversed_body = [self.mutate_stmt(stmt) for stmt in reversed(node.body)]
            reversed_body = irmutator.flatten(reversed_body)
            # loop_var -= 1
            reversed_body.append(
                loma_ir.Assign(
                    loop_var,
        ...
        def mutate_var(self, node):
            if self.in_assign:
                ...
                self.adj_accum_stmts += accum_deriv(
                    var_to_differential(node, self.var_to_dvar),
                    target_expr,
                    overwrite=False,
                )
        ...
        def mutate_sub(self, node):
            old_adj = self.adj
            left = self.mutate_expr(node.left)
            self.adj = loma_ir.BinaryOp(loma_ir.Sub(), loma_ir.ConstFloat(0.0), old_adj)
            right = self.mutate_expr(node.right)
            self.adj = old_adj
            return left + right
        ...
        def mutate_mul(self, node):
            ...
            # dz/dy = dz * x
            old_adj = self.adj
            self.adj = loma_ir.BinaryOp(loma_ir.Mul(), node.right, old_adj)
            left = self.mutate_expr(node.left)
            self.adj = loma_ir.BinaryOp(loma_ir.Mul(), node.left, old_adj)
            right = self.mutate_expr(node.right)
            self.adj = old_adj
            return left + right
        ...
        def mutate_div(self, node):
            ...
            # dz/dy = - dz * x / y^2
            old_adj = self.adj
            self.adj = loma_ir.BinaryOp(loma_ir.Div(), old_adj, node.right)
            left = self.mutate_expr(node.left)
            # - dz
            self.adj = loma_ir.BinaryOp(loma_ir.Sub(), loma_ir.ConstFloat(0.0), old_adj)
            # - dz * x
            self.adj = loma_ir.BinaryOp(loma_ir.Mul(), self.adj, node.left)
            # - dz * x / y^2
        ...
        def mutate_call(self, node):
            match node.id:
                case "sin":
                    ...
                    self.adj = loma_ir.BinaryOp(
                        loma_ir.Mul(),
                        loma_ir.Call("cos", node.args, lineno=node.lineno, t=node.t),
                        old_adj,
                        lineno=node.lineno,
                    )
                ...
                case "cos":
                    ...
                    self.adj = loma_ir.BinaryOp(
                        ...
                        loma_ir.BinaryOp(
                            loma_ir.Mul(),
                            loma_ir.Call(
                                "sin", node.args, lineno=node.lineno, t=node.t
                            ),
                            self.adj,
                            lineno=node.lineno,
                        ),
                ...
                case "sqrt":
                    ...
                    # dx = (1/2) * dy / y
                    old_adj = self.adj
                    sqrt = loma_ir.Call("sqrt", node.args, lineno=node.lineno, t=node.t)
                    self.adj = loma_ir.BinaryOp(
                        loma_ir.Mul(),
                    ...
                    self.adj = loma_ir.BinaryOp(
                        loma_ir.Div(), self.adj, sqrt, lineno=node.lineno
                    )
                    ret = self.mutate_expr(node.args[0])
                    self.adj = old_adj
                    return ret
                case "pow":
                    ...
                    # base term
                    self.adj = loma_ir.BinaryOp(
                        loma_ir.Mul(), self.adj, exp_expr, lineno=node.lineno
                    )
                    ...
                    pow_exp_minus_1 = loma_ir.Call(
                        "pow", [base_expr, exp_minus_1], lineno=node.lineno, t=node.t
                    )
                    self.adj = loma_ir.BinaryOp(
                        loma_ir.Mul(), self.adj, pow_exp_minus_1, lineno=node.lineno
                    )
                    ...
                    pow_expr = loma_ir.Call(
                        "pow", [base_expr, exp_expr], lineno=node.lineno, t=node.t
                    )
                    self.adj = loma_ir.BinaryOp(
                        loma_ir.Mul(), self.adj, pow_expr, lineno=node.lineno
                    )
                    log = loma_ir.Call("log", [base_expr], lineno=node.lineno)
                    self.adj = loma_ir.BinaryOp(
                        loma_ir.Mul(), self.adj, log, lineno=node.lineno
                    )
                ...
                case "exp":
                    assert len(node.args) == 1
                    exp = loma_ir.Call("exp", node.args, lineno=node.lineno, t=node.t)
                    old_adj = self.adj
                    self.adj = loma_ir.BinaryOp(
                        loma_ir.Mul(), self.adj, exp, lineno=node.lineno
                    )
                    ret = self.mutate_expr(node.args[0])
                    self.adj = old_adj
                    return ret
                case "log":
                    assert len(node.args) == 1
                    old_adj = self.adj
                    self.adj = loma_ir.BinaryOp(loma_ir.Div(), self.adj, node.args[0])
                    ret = self.mutate_expr(node.args[0])
                    self.adj = old_adj
                    return ret
                case "int2float":
                ...
                case "atomic_add":
                    # z = z + x
                    # -> dx += dz
                    target = var_to_differential(node.args[1], self.var_to_dvar)
                    source = var_to_differential(node.args[0], self.var_to_dvar)
                    return accum_deriv(target, source, overwrite=False)
                case "make__dfloat":
                    # z = make__dfloat(x, y)
                    old_adj = self.adj
                    self.adj = loma_ir.StructAccess(old_adj, "val")
                    ret0 = self.mutate_expr(node.args[0])
                    self.adj = loma_ir.StructAccess(old_adj, "dval")
                ...
                case _:  # user function
                    ...
                    for i, f_arg in enumerate(f.args):
                        ...
                        if f_arg.i == loma_ir.In():
                            ...
                            new_args.append(
                                loma_ir.Var(self.var_to_dvar[arg_expr.id], t=f_arg.t)
                            )
                        else:
                    ...
                    stmts.append(
                        loma_ir.CallStmt(
                            loma_ir.Call(func_to_rev[node.id], new_args, t=None)
                        )
                    )
                    return stmts

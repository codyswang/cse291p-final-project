...
def loma_to_ctypes_type(
    t: loma_ir.type | loma_ir.arg, ctypes_structs: dict[str, ctypes.Structure]
) -> ctypes.Structure:
    ...
    match t:
        ...
        case loma_ir.Array():
            base_type = loma_to_ctypes_type(t.t, ctypes_structs)
            if t.static_size is not None:
                return base_type * t.static_size
            else:
...
#include <math.h>
        ...
        if platform.system() == "Windows":
            ...
            log = run(
                ["cl.exe", "/c", "/O2", f"/Fo:{obj_filename}", tmp_c_filename],
                encoding="utf-8",
                capture_output=True,
            )
            if log.returncode != 0:
...
void atomic_add(float *ptr, float val) {
    ...
    do {
        ...
        code = codegen_slang.codegen_slang(
            structs,
            funcs,
        ...
        module = slangpy.Module.load_from_source(slang_device, name="_tmp", source=code)
        entry_points = [
            module.module.entry_point(func_name)
    ...
    if target == "c" or target == "ispc":
        ...
        for s in sorted_structs_list:
            structs[s.id] = type(
                ...
                {
                    "_fields_": [
                        (m.id, loma_to_ctypes_type(m.t, structs)) for m in s.members
                    ]
                },
            )

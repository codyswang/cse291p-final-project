...
def forward_diff(
    ...
    class FwdDiffMutator(irmutator.IRMutator):
        def mutate_function_def(self, node):
            ...
            new_args = [
                loma_ir.Arg(
                    arg.id, autodiff.type_to_diff_type(diff_structs, arg.t), arg.i
                )
        ...
        def mutate_return(self, node):
            ...
            if isinstance(self.ret_type, loma_ir.Float):
                ...
                call_expr = loma_ir.Call(
                    "make__dfloat", [val, dval], lineno=node.lineno
                )
            ...
            if node.val is not None:
                if isinstance(node.t, loma_ir.Float):
                    ...
                    return loma_ir.Declare(
                        node.target, diff_type, val=call_expr, lineno=node.lineno
                    )
                else:
                    val, _ = self.mutate_expr(node.val)
                    return loma_ir.Declare(
                        node.target, diff_type, val=val, lineno=node.lineno
                    )
            else:
                return loma_ir.Declare(node.target, diff_type, lineno=node.lineno)
        ...
        def mutate_call_stmt(self, node):
            ...
            if isinstance(val.t, loma_ir.Float):
                ...
                if dval != None:
                    return [loma_ir.CallStmt(val), loma_ir.CallStmt(dval)]
                else:
        ...
        def mutate_var(self, node):
            if isinstance(node.t, loma_ir.Float):
                return (
                    loma_ir.StructAccess(loma_ir.Var(node.id), "val", t=node.t),
        ...
        def mutate_array_access(self, node):
            ...
            if isinstance(node.t, loma_ir.Float):
                val = loma_ir.StructAccess(
                    loma_ir.ArrayAccess(array, index),
                    "val",
                    lineno=node.lineno,
                    t=node.t,
                )
                ...
                return (
                    loma_ir.ArrayAccess(array, index, lineno=node.lineno, t=node.t),
                    None,
                )
        ...
        def mutate_struct_access(self, node):
            ...
            if isinstance(node.t, loma_ir.Float):
                val = loma_ir.StructAccess(
                    loma_ir.StructAccess(struct, node.member_id),
                    "val",
                    lineno=node.lineno,
                    t=node.t,
                )
                ...
                return (
                    loma_ir.StructAccess(
                        struct, node.member_id, lineno=node.lineno, t=node.t
                    ),
                    None,
                )
        ...
        def mutate_add(self, node):
            ...
            val = loma_ir.BinaryOp(
                loma_ir.Add(), l_val, r_val, lineno=node.lineno, t=node.t
            )
            if l_dval is not None:
                assert r_dval is not None
                dval = loma_ir.BinaryOp(
                    loma_ir.Add(), l_dval, r_dval, lineno=node.lineno, t=node.t
                )
            else:
                dval = None
            return val, dval
        ...
        def mutate_sub(self, node):
            ...
            r_val, r_dval = self.mutate_expr(node.right)
            val = loma_ir.BinaryOp(
                loma_ir.Sub(), l_val, r_val, lineno=node.lineno, t=node.t
            )
            if l_dval is not None:
                assert r_dval is not None
                dval = loma_ir.BinaryOp(
                    loma_ir.Sub(), l_dval, r_dval, lineno=node.lineno, t=node.t
                )
            else:
                dval = None
            return val, dval
        ...
        def mutate_mul(self, node):
            ...
            r_val, r_dval = self.mutate_expr(node.right)
            val = loma_ir.BinaryOp(
                loma_ir.Mul(), l_val, r_val, lineno=node.lineno, t=node.t
            )
            if l_dval is not None:
                l_ = loma_ir.BinaryOp(loma_ir.Mul(), l_dval, r_val, lineno=node.lineno)
                r_ = loma_ir.BinaryOp(loma_ir.Mul(), l_val, r_dval, lineno=node.lineno)
                dval = loma_ir.BinaryOp(
                    loma_ir.Add(), l_, r_, lineno=node.lineno, t=node.t
                )
            else:
                dval = None
            return val, dval
        ...
        def mutate_div(self, node):
            ...
            r_val, r_dval = self.mutate_expr(node.right)
            val = loma_ir.BinaryOp(
                loma_ir.Div(), l_val, r_val, lineno=node.lineno, t=node.t
            )
            if l_dval is not None:
                denom = loma_ir.BinaryOp(loma_ir.Mul(), r_val, r_val)
                dl_pr = loma_ir.BinaryOp(
                    loma_ir.Mul(), l_dval, r_val, lineno=node.lineno
                )
                pl_dr = loma_ir.BinaryOp(
                    loma_ir.Mul(), l_val, r_dval, lineno=node.lineno
                )
                numerator = loma_ir.BinaryOp(
                    loma_ir.Sub(), dl_pr, pl_dr, lineno=node.lineno
                )
                dval = loma_ir.BinaryOp(
                    loma_ir.Div(), numerator, denom, lineno=node.lineno, t=node.t
                )
            return val, dval
        ...
        def mutate_less(self, node):
            ...
            return (
                loma_ir.BinaryOp(
                    loma_ir.Less(), l_val, r_val, lineno=node.lineno, t=node.t
                ),
                None,
            )
        ...
        def mutate_less_equal(self, node):
            ...
            return (
                loma_ir.BinaryOp(
                    loma_ir.LessEqual(), l_val, r_val, lineno=node.lineno, t=node.t
                ),
                None,
            )
        ...
        def mutate_greater(self, node):
            ...
            return (
                loma_ir.BinaryOp(
                    loma_ir.Greater(), l_val, r_val, lineno=node.lineno, t=node.t
                ),
                None,
            )
        ...
        def mutate_greater_equal(self, node):
            ...
            return (
                loma_ir.BinaryOp(
                    loma_ir.GreaterEqual(), l_val, r_val, lineno=node.lineno, t=node.t
                ),
                None,
            )
        ...
        def mutate_equal(self, node):
            ...
            return (
                loma_ir.BinaryOp(
                    loma_ir.Equal(), l_val, r_val, lineno=node.lineno, t=node.t
                ),
                None,
            )
        ...
        def mutate_call(self, node):
            ...
            match node.id:
                case "sin":
                    ...
                    val, dval = new_args[0]
                    sin = loma_ir.Call("sin", [val], lineno=node.lineno, t=node.t)
                    cos = loma_ir.Call("cos", [val], lineno=node.lineno, t=node.t)
                    return sin, loma_ir.BinaryOp(
                        loma_ir.Mul(), cos, dval, lineno=node.lineno, t=node.t
                    )
                case "cos":
                    assert len(new_args) == 1
                    val, dval = new_args[0]
                ...
                case "sqrt":
                    assert len(new_args) == 1
                    val, dval = new_args[0]
                    sqrt = loma_ir.Call("sqrt", [val], lineno=node.lineno, t=node.t)
                ...
                case "pow":
                    ...
                    pow_exp_minus_1 = loma_ir.Call(
                        "pow", [base_val, exp_minus_1], lineno=node.lineno, t=node.t
                    )
                    ...
                    pow = loma_ir.Call(
                        "pow", [base_val, exp_val], lineno=node.lineno, t=node.t
                    )
                    log = loma_ir.Call("log", [base_val], lineno=node.lineno, t=node.t)
                    pow_log = loma_ir.BinaryOp(
                        loma_ir.Mul(), pow, log, lineno=node.lineno, t=node.t
                    )
                    exp_dval_term = loma_ir.BinaryOp(
                        loma_ir.Mul(), exp_dval, pow_log, lineno=node.lineno, t=node.t
                    )
                ...
                case "exp":
                    assert len(new_args) == 1
                    val, dval = new_args[0]
                    exp = loma_ir.Call("exp", [val], lineno=node.lineno, t=node.t)
                    return exp, loma_ir.BinaryOp(
                        loma_ir.Mul(), dval, exp, lineno=node.lineno, t=node.t
                    )
                case "log":
                    assert len(new_args) == 1
                    val, dval = new_args[0]
                    log = loma_ir.Call("log", [val], lineno=node.lineno, t=node.t)
                ...
                case "int2float":
                    assert len(new_args) == 1
                    val, _ = new_args[0]
                    ret = loma_ir.Call("int2float", [val], lineno=node.lineno, t=node.t)
                    return ret, loma_ir.ConstFloat(0.0)
                case "float2int":
                    assert len(new_args) == 1
                    val, dval = new_args[0]
                    ret = loma_ir.Call("float2int", [val], lineno=node.lineno, t=node.t)
                    return ret, None
                case "thread_id":
                    return (
                        loma_ir.Call("thread_id", [], lineno=node.lineno, t=node.t),
                        None,
                    )
                case "atomic_add":
                    # x += y
                    # dx += dy
                    val0, dval0 = new_args[0]
                    val1, dval1 = new_args[1]
                    val_call = loma_ir.Call(
                        "atomic_add", [val0, val1], lineno=node.lineno, t=node.t
                    )
                    dval_call = loma_ir.Call(
                        "atomic_add", [dval0, dval1], lineno=node.lineno, t=node.t
                    )
                    return val_call, dval_call
                case _:  # user function
                    ...
                    if isinstance(node.t, loma_ir.Float):
                        val = loma_ir.StructAccess(
                            ret, "val", lineno=node.lineno, t=node.t
                        )
                        dval = loma_ir.StructAccess(
                            ret, "dval", lineno=node.lineno, t=node.t
                        )
                        return val, dval
                    else:
                        return ret, None
